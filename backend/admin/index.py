...
def get_user_revenue_details(email: str) -> List[Dict[str, Any]]:
    """Получить детализацию зарплаты пользователя по организациям с учётом rate_periods"""
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                SELECT 
                    ...
                    COALESCE(
                        (SELECT rp.contact_rate FROM t_p24058207_website_creation_pro.organization_rate_periods rp
                         WHERE rp.organization_id = ws.organization_id
                           AND rp.start_date <= ws.shift_date
                           AND (rp.end_date IS NULL OR rp.end_date >= ws.shift_date)
            ...
            for row in cur.fetchall():
                ...
                contacts = int(row[3])
                contact_rate = float(row[4]) if row[4] else 0
                payment_type = row[5] or 'per_contact'
                compensation = float(row[6]) if row[6] else 0
                ...
                if org_name not in org_salary_map:
                    org_salary_map[org_name] = {
                        ...
                        'shifts': 0,
                        'payment_type': payment_type,
                        'last_rate': contact_rate,
                    }

                if payment_type == 'per_contact':
                    if contact_rate > 0:
                        shift_salary = contacts * contact_rate
                    else:
                        shift_salary = contacts * 300 if contacts >= 10 else contacts * 200
                else:
                    shift_salary = compensation
                ...
                org_salary_map[org_name]['last_rate'] = contact_rate
                org_salary_map[org_name]['payment_type'] = payment_type

            org_revenues = []
            for org_name, data in org_salary_map.items():
                avg_rate = round(data['total_salary'] / data['total_contacts']) if data['total_contacts'] > 0 else int(data['last_rate'])
...
def _handle_request(event: Dict[str, Any], context: Any, method: str, headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if method == 'GET':
                ...
                with conn.cursor() as cur:
                    ...
                    for row in shifts_rows:
                        ...
                        shifts.append({
                            ...
                            'invoice_party': row[24] if row[24] else None,
                            'employee_status': row[25] if row[25] else 'employee',
                            'user_registered_at': row[26].isoformat() if row[26] else None,
                            'is_active': bool(row[27]) if row[27] is not None else True
                        })
        ...
        if action == 'set_senior':
            ...
            invoice_party = body_data.get('invoice_party', None)
                    ...
                    with conn.cursor() as cur:
                        cur.execute("""
                            ...
                            (user_id, work_date, organization_id, expense_amount, expense_comment, 
                             ...
                             invoice_paid, invoice_paid_date, personal_funds_amount, personal_funds_by_kms, personal_funds_by_kvv, compensation_amount, invoice_party, updated_at)
                            ...
                            DO UPDATE SET 
                                ...
                                invoice_party = EXCLUDED.invoice_party,
                                updated_at = CURRENT_TIMESTAMP
                        """, (user_id, work_date, organization_id, expense_amount, expense_comment,
                              paid_by_organization, paid_to_worker, salary_at_kvv, paid_kvv, paid_kms, invoice_issued, invoice_issued_date,
                              invoice_paid, invoice_paid_date, personal_funds_amount, personal_funds_by_kms, personal_funds_by_kvv, compensation_amount, invoice_party))
                        conn.commit()
