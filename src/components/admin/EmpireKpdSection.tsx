...
export default function EmpireKpdSection() {
  ...
  useEffect(() => {
    if (!isOpen || kpd) return;
    setLoading(true);
    fetch(`${TRAINING_API}?action=get_empire_kpd`, { headers: authHeaders() })
      .then(r => r.json())
      .then(d => { setKpd(d); setLoading(false); })
      .catch(() => setLoading(false));
  }, [isOpen]);

  const periodData = kpd ? (period === 'day' ? kpd.by_day : period === 'week' ? kpd.by_week : kpd.by_month) : [];
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="text-sm font-bold text-slate-800">Империя Промо</p>
            <p className="text-xs text-slate-400">Общая статистика КПД</p>
          </div>
        </div>
        <Icon name={isOpen ? 'ChevronUp' : 'ChevronDown'} size={18} className="text-slate-400 flex-shrink-0" />
      </button>
                    ...
                    {periodData.map((item, i) => {
                      ...
                      return (
                        <div key={key} className="rounded-xl overflow-hidden border border-slate-100">
                          <button
                            className={`w-full flex items-center justify-between px-4 py-3 transition-colors ${isOpenP ? 'bg-blue-50' : 'bg-white hover:bg-slate-50'}`}
                            onClick={() => togglePeriod(key)}
                          >
                            <div className="flex items-center gap-2">
                              <Icon name={isOpenP ? 'ChevronDown' : 'ChevronRight'} size={14} className="text-slate-400" />
                              <span className="text-sm font-medium text-slate-700">
                                {getLabel(item as { date?: string; week_start?: string; month_start?: string })}
                              </span>
                            </div>
                            <span className={`text-sm font-bold px-2.5 py-0.5 rounded-full ${isOpenP ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-600'}`}>
                              {item.count}
                            </span>
                          </button>
                                  ...
                                  {trainees.map(t => (
                                    <div key={t.id} className={`flex items-center justify-between px-4 py-2.5 ${!t.is_active ? 'opacity-50' : ''}`}>
                                      <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        {!t.is_active && <span className="text-[10px] text-slate-400 bg-slate-200 px-1.5 py-0.5 rounded-full flex-shrink-0">слив</span>}
                                      </div>
