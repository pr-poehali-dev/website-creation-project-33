import React from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import Icon from '@/components/ui/icon';

interface Organization {
  id: number;
  name: string;
  created_at: string;
  lead_count: number;
  contact_rate: number;
  payment_type: 'cash' | 'cashless';
}

interface OrganizationListItemProps {
  org: Organization;
  editingId: number | null;
  editingName: string;
  editingRate: string;
  editingPaymentType: 'cash' | 'cashless';
  updating: boolean;
  startEditing: (org: Organization) => void;
  cancelEditing: () => void;
  updateOrganization: (id: number) => void;
  deleteOrganization: (id: number, name: string) => void;
  setEditingName: (name: string) => void;
  setEditingRate: (rate: string) => void;
  setEditingPaymentType: (type: 'cash' | 'cashless') => void;
  onOpenPeriods: (org: { id: number; name: string }) => void;
}

export default function OrganizationListItem({
  org,
  editingId,
  editingName,
  editingRate,
  editingPaymentType,
  updating,
  startEditing,
  cancelEditing,
  updateOrganization,
  deleteOrganization,
  setEditingName,
  setEditingRate,
  setEditingPaymentType,
  onOpenPeriods
}: OrganizationListItemProps) {
  const isEditing = editingId === org.id;

  return (
    <div className="group p-3 md:p-4 rounded-xl bg-slate-800/50 hover:bg-slate-800 transition-all duration-200 border border-slate-700 hover:border-slate-600">
      {isEditing ? (
        <div className="space-y-3">
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Название</label>
            <Input
              value={editingName}
              onChange={(e) => setEditingName(e.target.value)}
              className="border-slate-700 bg-slate-900 text-slate-100 focus:border-slate-600 focus:ring-slate-600 h-9 text-sm"
            />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Ставка за контакт (₽)</label>
            <Input
              type="number"
              value={editingRate}
              onChange={(e) => setEditingRate(e.target.value)}
              className="border-slate-700 bg-slate-900 text-slate-100 focus:border-slate-600 focus:ring-slate-600 h-9 text-sm"
            />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-2 block">Тип оплаты</label>
            <div className="flex gap-2">
              <Button
                onClick={() => setEditingPaymentType('cash')}
                variant={editingPaymentType === 'cash' ? 'default' : 'outline'}
                size="sm"
                className={`flex-1 h-9 text-xs ${
                  editingPaymentType === 'cash'
                    ? 'bg-green-600 hover:bg-green-700 text-white'
                    : 'bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700'
                }`}
              >
                <Icon name="Banknote" size={14} className="mr-1.5" />
                Наличные
              </Button>
              <Button
                onClick={() => setEditingPaymentType('cashless')}
                variant={editingPaymentType === 'cashless' ? 'default' : 'outline'}
                size="sm"
                className={`flex-1 h-9 text-xs ${
                  editingPaymentType === 'cashless'
                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                    : 'bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700'
                }`}
              >
                <Icon name="CreditCard" size={14} className="mr-1.5" />
                Безнал
              </Button>
            </div>
          </div>
          <div className="flex gap-2 pt-2">
            <Button
              onClick={() => updateOrganization(org.id)}
              disabled={!editingName.trim() || updating}
              className="flex-1 bg-green-600 hover:bg-green-700 text-white h-9 text-xs"
            >
              {updating ? (
                <Icon name="Loader2" size={14} className="animate-spin" />
              ) : (
                <>
                  <Icon name="Check" size={14} className="mr-1.5" />
                  Сохранить
                </>
              )}
            </Button>
            <Button
              onClick={cancelEditing}
              disabled={updating}
              variant="outline"
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700 h-9 text-xs"
            >
              <Icon name="X" size={14} className="mr-1.5" />
              Отмена
            </Button>
          </div>
        </div>
      ) : (
        <>
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-slate-100 mb-1.5 text-sm md:text-base truncate">
                {org.name}
              </h3>
              <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm">
                <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/30 text-xs">
                  <Icon name="Users" size={12} className="mr-1" />
                  {org.lead_count} лидов
                </Badge>
                <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-xs">
                  <Icon name="DollarSign" size={12} className="mr-1" />
                  {org.contact_rate || 0} ₽
                </Badge>
                <Badge
                  className={
                    org.payment_type === 'cashless'
                      ? 'bg-blue-500/20 text-blue-400 border-blue-500/30 text-xs'
                      : 'bg-green-500/20 text-green-400 border-green-500/30 text-xs'
                  }
                >
                  <Icon
                    name={org.payment_type === 'cashless' ? 'CreditCard' : 'Banknote'}
                    size={12}
                    className="mr-1"
                  />
                  {org.payment_type === 'cashless' ? 'Безнал' : 'Наличные'}
                </Badge>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-1.5 md:gap-2">
            <Button
              onClick={() => onOpenPeriods({ id: org.id, name: org.name })}
              variant="outline"
              size="sm"
              className="flex-1 min-w-[120px] bg-slate-800 hover:bg-slate-700 text-slate-100 border-slate-700 transition-all duration-200 h-8 md:h-9 text-xs md:text-sm"
            >
              <Icon name="Calendar" size={12} className="mr-1 md:mr-1.5 md:w-[14px] md:h-[14px]" />
              <span className="hidden sm:inline">История ставок</span>
              <span className="sm:hidden">Ставки</span>
            </Button>
            <Button
              onClick={() => startEditing(org)}
              variant="outline"
              size="sm"
              className="flex-1 min-w-[100px] bg-slate-800 hover:bg-slate-700 text-slate-100 border-slate-700 transition-all duration-200 h-8 md:h-9 text-xs md:text-sm"
            >
              <Icon name="Pencil" size={12} className="mr-1 md:mr-1.5 md:w-[14px] md:h-[14px]" />
              <span className="hidden sm:inline">Изменить</span>
              <span className="sm:hidden">Ред.</span>
            </Button>
            <Button
              onClick={() => deleteOrganization(org.id, org.name)}
              variant="outline"
              size="sm"
              className="bg-red-500/10 hover:bg-red-500/20 text-red-400 border-red-500/30 hover:border-red-500/50 transition-all duration-200 h-8 md:h-9 px-2 md:px-4"
            >
              <Icon name="Trash2" size={12} className="md:w-[14px] md:h-[14px]" />
            </Button>
          </div>
        </>
      )}
    </div>
  );
}
