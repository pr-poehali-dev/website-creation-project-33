            ...
            <div>
              {users.map((userChat) => (
                <button
                  ...
                  className={`w-full p-3 md:p-4 text-left hover:bg-gray-50 active:bg-gray-100 transition-colors border-b ${
                    selectedUser?.id === userChat.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                  }`}
                >
                  <div className="flex items-start justify-between gap-2 md:gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-xs md:text-sm truncate">{userChat.name}</p>
                      ...
                      {userChat.last_message_time ? (
                        ...
                        <p className="text-[10px] md:text-xs text-gray-400 mt-1 italic">
                          Нет сообщений
                        </p>
                      )}
                    </div>
