...
import { getMetroDistanceKm, extractMetroFromOrgName } from './metroUtils';
  ...
  return (
    <div className="space-y-1.5">
      {availablePromoters.map(p => {
        ...
        const distKm = (orgMetro && promoterMetro) ? getMetroDistanceKm(promoterMetro, orgMetro) : null;
        const distLabel = distKm !== null
          ? distKm < 0.5 ? 'рядом' : `${distKm.toFixed(1)} км`
          : null;
        const distColor = distKm === null ? '' :
          distKm < 3 ? 'bg-green-50 text-green-600' :
          distKm < 8 ? 'bg-blue-50 text-blue-600' :
          distKm < 15 ? 'bg-amber-50 text-amber-600' :
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {promoterMetro && (
                <div className="text-[10px] text-gray-400 mt-0.5">🚇 {promoterMetro}</div>
              )}
            </div>

            <div className="flex items-center gap-2 flex-shrink-0">
              {distLabel && (
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${distColor}`}>
                  {distLabel}
                </span>
              )}
