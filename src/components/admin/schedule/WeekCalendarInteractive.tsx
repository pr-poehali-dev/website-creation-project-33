                ...
                <div className="w-full px-1 pb-1.5 space-y-0.5">
                  {dayPlans.map(plan => (
                    <div
                      ...
                      className="group/plan relative rounded-md px-1 py-0.5 text-[8px] md:text-[9px] font-semibold text-white truncate"
                      style={{ backgroundColor: plan.color }}
                      title={`${plan.organization_name}${plan.senior_name ? ' · ' + plan.senior_name : ''}${plan.contact_limit ? ' · ' + plan.contact_limit + ' кон.' : ''}`}
                    >
                      <span className="block truncate pr-6">{plan.organization_name}</span>
                      {plan.contact_limit && (
                        <span className="text-white/70 text-[7px] block">{plan.contact_limit} кон.</span>
                      )}
